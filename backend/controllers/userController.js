...
exports.loginUser = catchAsyncErrors(async (req, res, next) =>{
    ...
    const user = await User.findOne({ email }).select('+password'); //here we have to especially select password because we have set select: false in userModel.js
    if(!user){
...
exports.forgotPassword = catchAsyncErrors(async (req, res, next) =>{
    ...
    try{
        ...
        await sendEmail({
            email: user.email,
            subject: 'Ecommerce App - Password Reset Token',
            message
        });
...
exports.deleteUser = catchAsyncErrors(async (req, res, next) => {
    ...
    User.findById(req.params.id, function (err, user) {
        ...
        //we will remove user's image from cloudinary 
        const imageId = user.avatar.public_id;
